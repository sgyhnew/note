...
class MenuStack:  # 菜单栈，管理菜单状态
    def __init__(self, game):
        self.game = game
        self.stack : list[MenuData] = [MenuData('home')]  # 菜单栈初始化为首页
    ...
    def clear(self):    # 清空栈
        ...
        self.stack.append(MenuData('home'))  # 清空后回到首页
    
    def is_root(self) -> bool:           # 检查是否在根菜单（首页）
...
class Menu:       # 菜单系统，负责所有用户交互
    def __init__(self, game):
        ...
        self.menus = { # 所有菜单以及其配置
            'home':     self.menu_home(),      # 新增：游戏主界面
            'battle':   self.menu_battle(),    # 修改：原main改为battle
            'category': self.menu_category(),
            'level':    self.menu_level(),
        }
    
    def navigate_to(self, target_menu: str, context: Dict = None) -> bool:  
        try:
            # 确保状态栈有效
    ...
    def _update_state_selection(self, selected_key: str, options: Dict):  
    ...
    def _get_options_keys(self, options: Dict) -> List[str]:   
        return [k for k in options.keys() if k != 'z']

    def _build_skill_menu(self, category: str, level: str) -> Dict[str, Tuple[str, bool, int]]: 
    ...
    def _get_available_levels(self, category: str) -> List[str]:    
        levels = set()
        for skill_data in self.game._skill_cache.values():
    ...
    def _render_menu(self, options: Dict[str, Tuple[str, bool, int]],
                    title: str,
                    menu_type: str,
                    page: int = 0,
                    show_cost: bool = True) -> str: 
    ...
    def _run_menu(self, menu_type: str, context: dict = None) -> Any:
        current_state = self.get_current_state()
        ...
        # 构建选项
        options = config.build_options(self, context)
        ...
        # 处理选择
        result = config.handle_choice(self, choice, context)
        return result
    ...
    def menu_battle(self) -> MenuConfig:
        """战斗菜单（原main菜单）"""
        def build_options(menu, ctx):
            options = {
                'a': ('攻击', True, 0),
    ...
    def menu_category(self) -> MenuConfig:
        """行动类别菜单"""
        def build_options(menu, ctx):
            ...
            for i, level in enumerate(levels):
                key = chr(ord('a') + i)
                count = sum(1 for s in menu.game._skill_cache.values() 
                           if s.category == category and s.level == level)
    ...
    def menu_level(self) -> MenuConfig:
        """等级菜单"""
        def build_options(menu, ctx):
        ...
        def handle_choice(menu, choice, ctx):
            ...
            if choice in options and choice != 'z':
                ...
                # 选择成功，清空栈并返回技能名
                menu.stack.clear()
                menu.stack.push('home')  # 返回主界面而不是battle
                return skill_name
